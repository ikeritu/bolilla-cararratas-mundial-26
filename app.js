...
function renderCountdown(){
  ...
  if (diff <= 0) { el.textContent = "🔒 Bolilla cerrada. Ya solo queda llorar."; return; }
...
function bindUI(){
  ...
  $$(".avatar-option").forEach(btn => btn.addEventListener("click", () => {
    ...
    localStorage.setItem("cararratasAvatar", state.avatar);
    $$(".avatar-option").forEach(b=>b.classList.toggle("active", b.dataset.avatar === state.avatar));
  }));
  const savedAvatar = localStorage.getItem("cararratasAvatar");
...
function draftKey(){
  ...
  return `bolilla-cararratas:draft:${who}`;
}
let draftTimer = null;
...
function fillForm(pred){
  ...
  // v42: al volver a entrar, la predicción del servidor debe hidratar primero state.scores.
  // Las cajas de fase de grupos se re-renderizan por pestaña/grupo; si solo escribimos en los
  // inputs existentes, los grupos no visibles se pierden visualmente al navegar.
...
function readPreviousRankingSnapshot(){
  try { return JSON.parse(localStorage.getItem("bolilla-cararratas:rankingSnapshot") || "{}"); } catch(e){ return {}; }
}
function saveRankingSnapshot(rows){
  const snap = {};
  rows.forEach((r,i)=> snap[normTeam(r.name)] = { pos:i+1, total:Number(r.total||0) });
  localStorage.setItem("bolilla-cararratas:rankingSnapshot", JSON.stringify(snap));
}
...
function renderComparisonPanel(){
  ...
  if (!el) return;
  if (isEditOpen()) {
    el.innerHTML = `<div class="empty-state">Predicciones bloqueadas hasta el cierre. La rata no deja copiar deberes, ni aunque vengas con carita de inocente.</div>`;
    return;
  }
...
function renderFriendsPredictions(){
  ...
  if (!container) return;
  if (isEditOpen()) {
    container.innerHTML = `<p class="muted">Predicciones ocultas hasta el cierre. La rata no deja copiar deberes, que luego vienen los “yo ya lo tenía pensado”.</p>`;
    return;
  }
...
function renderGlobalStats(){
  ...
  if (isEditOpen()) { el.innerHTML = `<p class="muted">Estadísticas ocultas hasta el cierre. La rata guarda el salseo en una caja fuerte de queso.</p>`; return; }
  const preds = state.server.predictions || [];
...
function savePrediction(ev){
  ...
  // v43: el hash del justificante debe incluir si se guarda como borrador o como completa.
  const forHashAfterValidation = JSON.parse(JSON.stringify(prediction));
  if (forHashAfterValidation.meta) delete forHashAfterValidation.meta.predictionHash;
  prediction.meta.predictionHash = simpleHash(JSON.stringify(forHashAfterValidation));
  ...
  if (!validation.ok) {
    ...
    setMsg(
      ...
      "Borrador incompleto listo para guardar: faltan " +
        ...
        ". Puedes guardarlo y seguir otro día; la rata no juzga, solo guarda pruebas.",
      true
    );
  }
...
async function confirmSavePrediction(){
  ...
  try {
    ...
    renderMySummary();
    const okMsg = isDraft
      ? "Borrador guardado en Google Sheets. Puedes cerrar y volver luego; esta vez sí queda rastro, cararrata."
      : "La Rata ha hablado 🐀⚽. Tu porra queda guardada; luego no vengas con que el dedo se te resbaló.";
...
function renderRanking(){
  ...
  container.innerHTML = `
    <div class="ranking-hero">
      ...
      <div><span>Participantes</span><strong>${rows.length}</strong><em>cararatas en juego</em></div>
...
function renderCompare(){
  ...
  if (!el) return;
  if (isEditOpen()) { el.innerHTML = `<div class="empty-state">Comparativa bloqueada hasta el cierre. Primero se apuesta, luego se cotillea. Orden, ratas.</div>`; return; }
  const preds = state.server.predictions || [];
...
function renderReceipt(pred){
  ...
  </div><p class="muted">${m.completionStatus === "draft" ? "Borrador recuperable: puedes volver con tu PIN y terminarlo. La rata archiva hasta las medias verdades." : "Resguardo anti “yo no puse eso”. La rata archiva, audita y juzga en silencio."}</p>`;
}
function buildWhatsAppSummary(pred){
  ...
  return [
    "🐀 BOLILLA CARARATAS MUNDIAL 2026",
    "",
    ...
    "",
    "La rata ya ha hablado."
  ].filter(line=>line!=="").join("\n");
}
async function copyPredictionForWhatsApp(){
  const msg=document.getElementById("saveMsg");
  if(!state.prediction){ return setMsg(msg,"Primero guarda la porra. La rata no firma servilletas en blanco.",false); }
...
function renderCommandCenter(){
  ...
  if(isEditOpen()){ el.innerHTML=`<div class="empty-state">Centro de mando cerrado hasta el fin del plazo. La rata no filtra encuestas a pie de urna.</div>`; return; }
  const preds=state.server.predictions||[];
  if(!preds.length){ el.innerHTML=`<div class="empty-state">Centro de mando sin datos. La rata tiene la pizarra limpia y la paciencia justa.</div>`; return; }
  const champions=countValues(preds.map(p=>p.prediction?.knockout?.champion?.[0]));
  const boots=countValues(preds.map(p=>p.prediction?.awards?.goldenBoot?.[0]));
  ...
  el.innerHTML=`<div class="command-grid">
    ...
    ${statCard("🐑 Rata mainstream", mainstream ? mainstream.name : "—")}
    ${statCard("🎲 Rata del caos", chaos ? chaos.name : "—")}
    ${statCard("💸 Morosos", morosos.length ? morosos.join(", ") : "Nadie. Milagro.")}
...
function fifaRankOrder(team){ return FIFA_RANKING_FALLBACK[team] || 999; }
function groupTeamOrder(team){ for (const g of GROUP_ORDER) { const idx = (WC_DATA.groups[g] || []).indexOf(team); if (idx >= 0) return idx; } return 99; }
function compareThirdTeamsFifa(a,b){ return (b.pts-a.pts) || (b.gd-a.gd) || (b.gf-a.gf) || (fifaRankOrder(a.team)-fifaRankOrder(b.team)) || groupTeamOrder(a.team)-groupTeamOrder(b.team) || a.team.localeCompare(b.team); }
...
function headToHeadMiniTable(group, teams){
  const set = new Set(teams); const mini = {}; teams.forEach(team => mini[team] = { team, pts:0, gf:0, ga:0, gd:0 });
  WC_DATA.matches.filter(m => m.group === group && set.has(m.home) && set.has(m.away)).forEach(m => {
    const sc = state.scores[m.id] || {}; if (sc.home === null || sc.home === undefined || sc.away === null || sc.away === undefined) return;
    ...
    if (h > a) mini[m.home].pts += 3; else if (h < a) mini[m.away].pts += 3; else { mini[m.home].pts += 1; mini[m.away].pts += 1; }
  });
  return mini;
}
...
function renderBracket(){
  ...
  if (notice) notice.textContent = enough
    ? "Dieciseisavos calculados desde grupos. Primeros y segundos van por cuadro FIFA; los terceros usan asignación compatible hasta cargar la matriz completa. La rata avisa, que luego vienen los peritos."
...
function collectPrediction(){
  ...
  // v34: MUY IMPORTANTE.
  // Antes se llamaba a syncBracketWithGroups() justo antes de guardar. Esa función recalcula
  // dieciseisavos y limpia rondas posteriores para evitar inconsistencias visuales.
  // Al usarla en el guardado podía dejar la predicción persistida solo con grupos/r32 si el
  // bracket se recomponía en ese momento. Para guardar, no se debe mutar state.knockout;
  // se toma una foto exacta de lo que el usuario ha seleccionado.
  const ko = state.knockout || {};
